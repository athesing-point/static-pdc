(() => {
  ...
  var ENABLE_BACKUP_MODE = false;
  var ROBOTS_TXT = ENABLE_BACKUP_MODE ? `User-agent: *
Allow: /
` : `User-agent: *
Disallow: /
...
`;
  ...
  var mapRequestToAsset = /* @__PURE__ */ __name((request) => {
    const parsedUrl = new URL(request.url);
    let pathname = parsedUrl.pathname;
    if (pathname.endsWith("/")) {
      pathname += "index.html";
  ...
  async function handleEvent(event) {
    ...
    if (/^\/blog(\/.*)?$/.test(pathname) || pathname === "/blog" || pathname === "/blog/" || /^\/blog\/category(\/.*)?$/.test(pathname) || pathname === "/blog/category" || pathname === "/blog/category/" || /^\/home-equity(\/.*)?$/.test(pathname) || pathname === "/home-equity" || pathname === "/home-equity/" || /^\/partner(\/.*)?$/.test(pathname) || pathname === "/partner" || pathname === "/partner/" || /^\/media-room(\/.*)?$/.test(pathname) || pathname === "/media-room" || pathname === "/media-room/") {
    ...
    try {
      let response = await (0, import_kv_asset_handler.getAssetFromKV)(event, { mapRequestToAsset });
